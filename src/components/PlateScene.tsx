...
export function PlateScene({ svgWidth, svgHeight }: PlateSceneProps) {
  ...
  const yMinBox   = plateMidY - 3;    // ≈ −3.707 ft (behind camera)
  const yMaxBox   = plateMidY + 3;    // ≈ +2.293 ft (toward pitcher)
  ...
  const leftBox = toPoints(nearClipY(
    [{ x: -BI, y: yMaxBox }, { x: -BO, y: yMaxBox },
     { x: -BO, y: yMinBox }, { x: -BI, y: yMinBox }],
    NEAR_Y,
  ));
  const rightBox = toPoints(nearClipY(
    [{ x:  BI, y: yMaxBox }, { x:  BO, y: yMaxBox },
     { x:  BO, y: yMinBox }, { x:  BI, y: yMinBox }],
    NEAR_Y,
  ));
  ...
  const foulDist = 100;
  const foulREnd = p( foulDist / Math.SQRT2, APEX_Y + foulDist / Math.SQRT2, 0);
  const foulLEnd = p(-foulDist / Math.SQRT2, APEX_Y + foulDist / Math.SQRT2, 0);
  ...
  return (
    ...
    >
      <defs>
        ...
        <linearGradient id="ps-fr" gradientUnits="userSpaceOnUse"
          x1={pFSide.x} y1={pFSide.y}
          x2={foulREnd.x} y2={foulREnd.y}
        >
          <stop offset="0%"   stopColor="#d8cead" stopOpacity="0.42" />
          <stop offset="100%" stopColor="#d8cead" stopOpacity="0"    />
        </linearGradient>
        <linearGradient id="ps-fl" gradientUnits="userSpaceOnUse"
          x1={pTSide.x} y1={pTSide.y}
          x2={foulLEnd.x} y2={foulLEnd.y}
        >
          <stop offset="0%"   stopColor="#d8cead" stopOpacity="0.42" />
          <stop offset="100%" stopColor="#d8cead" stopOpacity="0"    />
        </linearGradient>
      ...
      <ellipse
        cx={cx} cy={svgHeight}
        rx={svgWidth * 0.44} ry={svgHeight * 0.60}
        fill="url(#ps-dirt)"
      />
      ...
      <line
        x1={pFSide.x} y1={pFSide.y}
        x2={foulREnd.x} y2={foulREnd.y}
        stroke="url(#ps-fr)" strokeWidth={1.5}
        clipPath="url(#ps-vp)"
      />
      <line
        x1={pTSide.x} y1={pTSide.y}
        x2={foulLEnd.x} y2={foulLEnd.y}
        stroke="url(#ps-fl)" strokeWidth={1.5}
        clipPath="url(#ps-vp)"
      />
